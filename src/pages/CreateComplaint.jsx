...
function CreateComplaint() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const complaintData = { title, description, category, location };
  ...
  const inputClass = "w-full px-6 py-4 border-2 border-orange-200 rounded-xl focus:outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-100 transition-all duration-200 font-medium text-lg text-gray-800 bg-white group-hover:border-orange-400";
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 py-12 px-4">
        <div className="max-w-2xl mx-auto">
          <div className="bg-gradient-to-r from-orange-500 to-red-500 rounded-3xl shadow-xl p-10 text-white mb-10 hover:shadow-2xl transition-all duration-300">
            <h1 className="text-5xl font-black mb-3">📝 Report an Issue</h1>
            <p className="text-orange-100 text-lg font-semibold">Help us fix your city - every report matters!</p>
          </div>

          <div className="bg-white rounded-3xl shadow-2xl p-10 border-2 border-orange-100 hover:shadow-3xl transition-all duration-300">
            <form onSubmit={handleSubmit} className="space-y-7">
              ...
              <div className="border-2 border-dashed border-orange-300 rounded-xl p-8 text-center hover:border-orange-500 transition-colors group">
                <label className="block text-gray-800 font-bold mb-3 text-lg">📸 Attach Photo (Optional)</label>
                <input type="file" onChange={(e) => setImage(e.target.files[0])} className="hidden" id="image-upload" />
                <label htmlFor="image-upload" className="cursor-pointer block group-hover:text-orange-600 transition-colors">
                  ...
                  {image && <p className="text-orange-500 font-semibold mt-2">✅ {image.name}</p>}
                </label>
              </div>

              <button type="submit" className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white py-4 rounded-xl font-bold text-xl hover:shadow-2xl hover:scale-105 transition-all duration-200 uppercase tracking-wide mt-4 relative overflow-hidden group">
